...
from send_email import SendEmail1
...
def password_entry_system():
    ...
    def check_password():
        global wrong_attempts, max_attempts
        ...
        if len(entered_password) >= 6 and entered_password[-6:] == correct_password:
            ...
            unlock_door()    # Gọi hàm mở khóa cửa hiển thị thông báo "cửa đã được mở khóa"
            ...
            wrong_attempts = 0  # Reset số lần nhập sai khi nhập đúng
        else:
             # Mật khẩu sai
            wrong_attempts += 1  # Tăng số lần nhập sai
            clear_password()
            activate_incorrect_alarm()
            root.after(2000,lambda: deactivate_alarm())
            if wrong_attempts >= max_attempts:
                # Đã vượt quá số lần nhập sai cho phép
                show_message(f"Đã nhập sai {max_attempts} lần! Gửi cảnh báo.")
                
                SendEmail1(None)  # Gọi hàm gửi email cảnh báo
                wrong_attempts = 0  # Reset số lần nhập sai
            else:
                show_message(f"Sai mật khẩu! Nhập lại. Số lần nhập sai: {wrong_attempts}/{max_attempts}")
            root.after(3000,lambda: clear_password())
            
             # Thêm đoạn này để quay lại menu chính sau khi mở khóa
            

    def auto_lock_door():
        """Hàm tự động khóa cửa sau 5 giây nếu nó đã được mở."""
        global doorUnlock, prevTime
