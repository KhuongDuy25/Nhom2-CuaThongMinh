...
def unlock_with_face(data):
    ...
    while True:
        ...
        for d in dets:
            ...
            if name == "Unknown":
                ...
                if unknown_count > unknown_threshold:
                    ...
                    SendEmail(os.path.join(image_path))  # Gọi hàm gửi email cảnh báo với hình ảnh đính kèm
            ...
            if name != "Unknown" and (time.time() - last_recognition_time) > recognition_cooldown:
                ...
                last_recognition_time = time.time()  # Cập nhật thời gian nhận diện
                            # Tạo timer để tự động khóa sau 10 giây
                def auto_lock():
                    nonlocal doorUnlock
                    if doorUnlock:
                        doorUnlock = False
                        lock_door()
                threading.Timer(10.0, auto_lock).start()
        ...
        if get_door_status() == False:  # Kiểm tra trạng thái cửa
            ...
            if doorUnlock  and time.time() - prevTime > 10:  # Nếu không có khuôn mặt
                doorUnlock = False
                lock_door()  # Khóa cửa
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            if doorUnlock:
                lock_door()  # Khóa cửa nếu cửa đang mở
